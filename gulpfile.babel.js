...
var settings = {
  jadeLayouts: 'jadefiles/layouts/',
  jadeIncludes: 'jadefiles/includes/',
  includesDir: '_source/_includes/',
  layoutsDir: '_source/_layouts/',
  jadeSections: 'jadefiles/home-sections/',
  homeSections: '_source/_home_sections/'
};
...
// wait for jade to compile, use jekyll to build site and compile sass to _site/
gulp.task('jekyll', ['jade'], function(cb) {
  var build = require('child_process').spawn('jekyll', ['build'], {stdio: 'inherit'})
  build.on('exit', cb);
});

// here on optimizes _site/ and pipes to dist/
// Lint JavaScript
gulp.task('lint', () =>
...
gulp.task('default', ['jekyll'], () => {
  ...
  gulp.watch(['_source/images/**/*'], reload);
  gulp.watch(['_source/**/*.markdown'], ['jekyll', reload]);
...
gulp.task('pagespeed', cb =>
  ...
  pagespeed('jwarner3412.github.io', {
    strategy: 'mobile'
...
gulp.task('copy-sw-scripts', () => {
  return gulp.src(['node_modules/sw-toolbox/sw-toolbox.js', '_site/scripts/sw/runtime-caching.js'])
