## My custom shiz
Jekyll & Jade & Gulp.babel
Oh, and Bourbon, neat, bitters, and normalize pre-installed.
jade support for includes and layouts. re-arranged gulpfile a bit, gulp default serves ./_site (built jekyll site unoptomized), gulp build:dist and gulp serve:dist optimize jekyll built site with googles scripts into ./dist/. Google head matter brought into ./_includes/head.html, body script stuff brought into default layout. May be a good idea to re-install bourbon, got an odd error on the initial add --all. I'm a noob, so if i can do better please let me know. Enjoy.
